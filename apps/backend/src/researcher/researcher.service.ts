...
export class ResearcherService {
  ...
  runFlTest(strategy = 'fedscrt', rounds = 10, alpha = 0.5) {
    ...
    const exp = this.getFlExperiments().find(
      (e) => e.strategy === s && e.alpha === a,
    );
   ...
   */
  async getNodeAudit(flClientId: string): Promise<any> {
   ...
   */
  async getInsights(limit = 10): Promise<any> {
    ...
    for (const c of cases) {
      events.push({
        ...
        kind: 'case',
        title: 'Scan analysed',
        detail: `${c.hospital?.displayName ?? 'Patient node'} · ${c.predictedSubtype} (${Math.round(
          c.confidence * 100,
        )}%)`,
    ...
    for (const r of rounds) {
      events.push({
        ...
        detail: `Round ${r.roundNumber} (${r.strategy === 'FEDAVG' ? 'FedAvg' : 'FedSCRT'}) · macro-F1 ${r.globalF1After.toFixed(
          3,
        )}`,
